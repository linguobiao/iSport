...
public class AreaChart extends LnChart {
	...
	private boolean isTwoXiao=false;
				...
				if(dotInfo.mValue>0){
					if(isTwoXiao){
						bd.getLabelOptions().drawLabel(canvas, pLine.getDotLabelPaint(),
								df.format(dotInfo.mValue)+"", dotInfo.mX,
								dotInfo.mY, itemAngle, bd.getLineColor());
					}else{
						bd.getLabelOptions().drawLabel(canvas, pLine.getDotLabelPaint(),
								(dotInfo.mValue+"").replace(".0", ""), dotInfo.mX,
								dotInfo.mY, itemAngle, bd.getLineColor());
					}
	...
	public boolean isTwoXiao() {
		return isTwoXiao;
	}

	public void setTwoXiao(boolean isTwoXiao) {
		this.isTwoXiao = isTwoXiao;
	}
